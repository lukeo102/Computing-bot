...
async def timeout_start(ctx: commands.context.Context, log: Log):
    try:
        log.append_log("Timeout command received")

        reply_message = await ctx.send("Working")

        error = verify_command(ctx=ctx, role_allowed='mods', no_parameters=2, command="timeout")
        if error:
        ...
        if not message[1] in roles.keys():
            log.append_log(f"Requested role ({message[1]}) does not exist")
        ...
        for member in members:
            try:
                ...
                log.append_log(f"Failed to timeout user {member}, reason: {e}")
        ...
        log.append_log("Timeout complete")
